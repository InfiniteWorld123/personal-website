...
> Read `AGENTS.md`, `docs/v2/foundation.md`, and the complete approved `docs/v2/services.md`. Inspect current Backend2/Auth, public Services/Home routes, and unrelated concurrent changes. Services V2 is an independent owner-managed catalogue, not a lead, invoice, checkout, subscription, referral, or booking workflow. The current PDFs and public content are references only; do not silently import them or change live prices. If a material decision is still open, ask the owner first. Otherwise implement the full Backend2 phase with independent service records, trilingual draft/publish rules, private pending edits, public snapshots, manual order, featured flag, approved price/offer presentation, owner auth, bounded pagination, SEO data, tests, and real runtime verification. Report what was and was not verified; stop before production frontend.
>

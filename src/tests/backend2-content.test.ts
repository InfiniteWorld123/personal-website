...
const importer = await import('#/backend2/modules/content/content.import')
const exporter = await import('#/backend2/db/content-export-legacy')
...
/*
 * Generous timeouts: each of these writes the whole registry — 734 rows — into
 * a PostgreSQL running in WebAssembly, and a busy machine makes that slow.
 */
const IMPORT_TIMEOUT = { timeout: 60_000 }

describe('the one-time import', () => {
  const exported = (over: Partial<Record<'de' | 'en' | 'ar' | 'shared', Json>> = {}, ignoredDrafts = 2) =>
    exporter.toLegacyExport(
      [
        { key: HEADLINE, language: 'de', published_value: 'Legacy-Überschrift' },
        { key: 'home.hero.typed[]', language: 'en', published_value: ['Shops', 'Apps'] },
        { key: 'site.email', language: '*', published_value: 'hello@example.de' },
        { key: 'price.websites', language: '*', published_value: 1200 },
        { key: 'services.items.websites.name', language: 'de', published_value: 'Webseiten' },
        { key: 'work.items.inknest.summary', language: 'en', published_value: 'A summary' },
        { key: 'shell.nav', language: 'de', published_value: 'Nav' },
        { key: HEADLINE, language: '*', published_value: 'Wrong slot' },
        ...Object.entries(over).flatMap(([language, values]) =>
          Object.entries(values ?? {}).map(([key, value]) => ({
            key,
            language: language === 'shared' ? '*' : language,
  ...
  it('plans every registry field in every language, and explains everything it leaves out', () => {
    const plan = importer.planContentImport(importer.readLegacyExport(exported()))
    const { counts } = plan.manifest

    const perLanguage = registry.contentRegistry.filter((field) => !field.shared).length
    expect(counts.rows).toBe(perLanguage * 3 + 5)
    expect(counts.perSlot.de).toEqual({ rows: perLanguage, fromLegacyOverride: 1 })
    expect(counts.perSlot.shared).toEqual({ rows: 5, fromLegacyOverride: 1 })
    expect(counts.fromLegacyOverride).toBe(3)
    expect(counts.invalid).toBe(0)
    expect(plan.manifest.source.ignoredDrafts).toBe(2)

    const excluded = plan.manifest.excluded.map((item) => `${item.language} ${item.key} ${item.reason}`)
    expect(excluded.sort()).toEqual([
      'de services.items.websites.name owned-by-services',
      'de shell.nav not-in-v2-registry',
      'en work.items.inknest.summary owned-by-projects',
      'shared home.hero.headline wrong-language-slot',
      'shared price.websites owned-by-services',
    ])
    // A dynamic module's published legacy wording is named, so its own cutover keeps it.
    const services = plan.manifest.excluded.find((item) => item.key === 'services.items.websites.name')
    expect(services?.note).toContain('Webseiten')

    expect(plan.manifest.differsFromDefault.map((item) => `${item.language} ${item.key}`).sort()).toEqual([
      `de ${HEADLINE}`,
      'en home.hero.typed[]',
      'shared site.email',
    ])
  ...
  it('refuses a file that is not an export', () => {
    expect(() => importer.readLegacyExport({ format: 'something-else' })).toThrow()
  })

  it('writes the effective wording in one transaction, once, and visitors read it', IMPORT_TIMEOUT, async () => {
    const plan = importer.planContentImport(importer.readLegacyExport(exported()))
    const applied = await run(() => importer.applyContentImport(plan, 'test export'))

    expect(applied.written).toBe(plan.manifest.counts.rows)
    expect((await publicCopy('de')).fields[HEADLINE]).toBe('Legacy-Überschrift')
    expect((await publicCopy('en')).fields['home.hero.typed[]']).toEqual(['Shops', 'Apps'])
    expect((await publicCopy('ar')).shared['site.email']).toBe('hello@example.de')
    expect((await publicCopy('en')).fields[HEADLINE]).toBe(en.home.hero.headline)

    // History starts empty: an import is not an edit.
    expect((await call('GET', '/owner/content/history')).body.data.total).toBe(0)

    // Imported fields are at revision 1, and Original still knows the release wording.
    const slot = await slotOf(HEADLINE, 'de')
    expect(slot).toMatchObject({ revision: 1, isOriginal: false, original: de.home.hero.headline })

    await expect(run(() => importer.applyContentImport(plan, 'again'))).rejects.toMatchObject({ code: 'CONFLICT' })
  })

  it('writes nothing when any value fails the V2 rules', async () => {
    const plan = importer.planContentImport(importer.readLegacyExport(exported({ de: { 'home.hero.sub': '   ' } })))

    expect(plan.manifest.invalid).toEqual([{ key: 'home.hero.sub', language: 'de', message: 'This text cannot be empty' }])
    await expect(run(() => importer.applyContentImport(plan, 'bad'))).rejects.toMatchObject({ code: 'CONFLICT' })

    const { rows } = await database.db.query('SELECT count(*)::int AS total FROM v2_content_values')
    expect(rows[0].total).toBe(0)
  })

  it('compares V2 with a fresh export before cutover', IMPORT_TIMEOUT, async () => {
    const plan = importer.planContentImport(importer.readLegacyExport(exported()))
    await run(() => importer.applyContentImport(plan, 'test export'))

    expect(await run(() => importer.compareContentImport(plan))).toEqual([])

    // The legacy site changed while V2 was being built.
    const later = importer.planContentImport(importer.readLegacyExport(exported({ en: { 'home.hero.sub': 'Changed later' } })))
    expect(await run(() => importer.compareContentImport(later))).toEqual([
      { key: 'home.hero.sub', language: 'en', v2: en.home.hero.sub, legacy: 'Changed later' },
    ])
  })

  it('rolls back an untouched import, and refuses once a field was edited since', IMPORT_TIMEOUT, async () => {
    const plan = importer.planContentImport(importer.readLegacyExport(exported()))
    const { importId } = await run(() => importer.applyContentImport(plan, 'test export'))

    await save(HEADLINE, 'de', 'Nach dem Import', 1)
    await expect(run(() => importer.rollbackContentImport(importId))).rejects.toMatchObject({ code: 'CONFLICT' })
    expect((await publicCopy('de')).fields[HEADLINE]).toBe('Nach dem Import')

    await database.reset()
    const second = await run(() => importer.applyContentImport(plan, 'test export'))
    const rolled = await run(() => importer.rollbackContentImport(second.importId))

    expect(rolled.removed).toBe(plan.manifest.counts.rows)
    expect((await publicCopy('de')).fields[HEADLINE]).toBe(de.home.hero.headline)
    await expect(run(() => importer.rollbackContentImport(second.importId))).rejects.toMatchObject({ code: 'CONFLICT' })
  })

  it('reads the legacy database only inside a read-only transaction, and never opens V2', async () => {
    const source = await readFile(new URL('../backend2/db/content-export-legacy.ts', import.meta.url), 'utf8')

    expect(source).toContain("'BEGIN TRANSACTION READ ONLY'")
    expect(source).not.toMatch(/\b(INSERT|UPDATE|DELETE|TRUNCATE|DROP|ALTER)\b/)
    expect(source).not.toContain('DATABASE_URL_V2')
    expect(source).not.toContain("from './client'")
    // Drafts are counted, never exported.
    expect(exporter.LEGACY_EXPORT_SQL).toContain('published_value IS NOT NULL')
    expect(exporter.LEGACY_EXPORT_SQL).not.toContain('draft_value')

    const importSource = await readFile(new URL('../backend2/modules/content/content.import.ts', import.meta.url), 'utf8')
    expect(importSource).not.toContain("from 'pg'")
  })
})

...
const OVERVIEW: AnalyticsOverviewResponse = {
  ...
  headline: [
    ...
    metric('invoices.overdue', { value: 2, amounts: [{ currency: 'EUR', minor: 69000 }] }),
...
describe('the Overview', () => {
  it('shows the four headline figures honestly, each currency apart', async () => {
    answer('/api/v2/owner/analytics/overview?period=30d', { body: OVERVIEW })
    answer('/api/v2/owner/analytics/sections/sales?period=7d', { body: SALES })
    answer('/api/v2/owner/analytics/sections/operations?period=7d', { status: 500, body: { success: false } })
    renderWithClient(<OverviewPage name="Yaman Warda" />)

    const received = await screen.findByRole('region', { name: 'Received' })

    await waitFor(() => expect(received.textContent).toContain('€895.00'))
    expect(received.textContent).toContain('US$1,000.00')
    expect(received.textContent).toContain('EUR −55% vs the 30 days before')

    expect(screen.getByRole('region', { name: 'Overdue' }).textContent).toContain('€690.00 still to pay')

    const visitors = screen.getByRole('region', { name: 'Visitors' })

    expect(within(visitors).getByText('Not connected')).toBeTruthy()
    expect(visitors.textContent).not.toMatch(/\b0\b/)

    const unread = screen.getByRole('region', { name: 'Unread inbox' })

    expect(within(unread).getByText('Could not load')).toBeTruthy()
  })

  it('lists what needs the owner, and keeps a failing week section to its own rows', { timeout: 20_000 }, async () => {
    answer('/api/v2/owner/analytics/overview?period=30d', { body: OVERVIEW })
    answer('/api/v2/owner/analytics/sections/sales?period=7d', { body: SALES })
    answer('/api/v2/owner/analytics/sections/operations?period=7d', { status: 500, body: { success: false } })
    renderWithClient(<OverviewPage name="Yaman Warda" />)

    expect(await screen.findByText('2 follow-ups are due')).toBeTruthy()
    expect(screen.getByText('No appointments in the next 7 days')).toBeTruthy()
    expect(screen.getByText('Inbox: could not load')).toBeTruthy()

    expect(await screen.findByText('6 new leads')).toBeTruthy()
    expect(screen.getByText('3 won · 2 lost')).toBeTruthy()
    await waitFor(() => expect(screen.getByText('Bookings: could not load')).toBeTruthy(), { timeout: 8000 })
    expect(screen.getByRole('link', { name: /Open analytics/ }).getAttribute('href')).toBe('/dashboard/analytics')
  })

  it('shows one alert, not zeros, when the headline figures fail', { timeout: 20_000 }, async () => {
    answer('/api/v2/owner/analytics/overview?period=30d', { status: 500, body: { success: false } })
    answer('/api/v2/owner/analytics/sections/sales?period=7d', 'hang')
    answer('/api/v2/owner/analytics/sections/operations?period=7d', 'hang')
    renderWithClient(<OverviewPage name="Yaman Warda" />)

    expect(await screen.findByText('The headline figures could not be loaded.', {}, { timeout: 8000 })).toBeTruthy()
    expect(screen.getAllByText('Could not load')).toHaveLength(4)

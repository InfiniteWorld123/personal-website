...
/** Three openings the owner reaches for often enough to keep one click away. */
const SNIPPETS = [
  { label: 'Propose a call', text: 'Would a short call work? Here are three times that suit me this week:' },
  { label: 'Ask about budget', text: 'Before I put a number on this: what budget range are you working with?' },
  { label: 'Send an offer', text: 'Here is my proposal, valid for 14 days:' },
]
...
export function LeadReadingPane({
  ...
  return (
    <div className="flex min-w-0 flex-col gap-5 p-4 sm:p-6">
      <div className="border-border flex flex-wrap items-start gap-3 border-b pb-4">
        ...
        <div className="text-end">
          ...
          <p className="text-muted-foreground mt-1 text-xs">{formatFull(lead.createdAt)}</p>
        </div>
      </div>
        ...
        <dl className="border-border bg-muted/40 grid gap-4 rounded-lg border p-4 sm:grid-cols-3">
          {facts.map((fact) => (
            <div key={fact.label}>
              ...
              <dd className="mt-0.5 text-sm font-medium" dir={fact.label === 'Phone' ? 'ltr' : undefined}>
                {fact.value}
              </dd>
            </div>
          ))}
      ...
      {lead.bookings.length > 0 ? (
        <div className="border-primary/30 text-primary flex flex-wrap items-center gap-2 rounded-lg border px-3 py-2 text-sm">
          <CalendarClock aria-hidden="true" className="size-4" />
          {lead.bookings.map((booking) => (
            <span key={booking.id}>
              {booking.label} · {formatBerlin(booking.startsAt)}
              {booking.status !== 'CONFIRMED' ? ` (${booking.status.toLowerCase()})` : ''}
            </span>
          ))}
        </div>
      ) : null}
      ...
      <div>
        <h2 className="text-lg font-semibold" dir={isArabic(lead.language) ? 'rtl' : undefined}>
          {lead.subject}
        </h2>
        ...
        <p
          ...
          dir={isArabic(lead.language) ? 'rtl' : undefined}
        >
          {lead.message}
        </p>
      </div>
        ...
        <div className="flex flex-col gap-3">
          {thread.map((message) => (
            ...
            >
              ...
              <p className="whitespace-pre-wrap" dir={isArabic(lead.language) ? 'rtl' : undefined}>
                {message.body}
              </p>
            </div>
          ))}
      ...
      <div className="border-border rounded-lg border p-3">
          ...
          <div className="mb-2 flex flex-wrap gap-2">
            {SNIPPETS.map((snippet) => (
              <button
                ...
                className="border-border text-muted-foreground hover:bg-accent rounded-full border px-3 py-1 text-xs"
              >
                {snippet.label}
              </button>
            ))}
        ...
        <Textarea
          ...
          placeholder={canSendMail ? 'Write your reply…' : 'Email is not configured, so replies cannot be sent'}
        ...
        {preferences.signature ? (
          <p className="text-muted-foreground border-border mt-2 border-t border-dashed pt-2 text-xs leading-5 whitespace-pre-line">
          ...
          <div className="mt-2 flex flex-col gap-2">
            {lead.notes.map((entry) => (
              <div key={entry.id} className="flex items-start gap-2 text-sm leading-6">
                <p className="flex-1 whitespace-pre-wrap">{entry.body}</p>

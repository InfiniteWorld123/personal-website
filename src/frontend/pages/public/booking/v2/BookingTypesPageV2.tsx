...
import type { PublicBookingType } from '#/backend2/contracts/booking.contract'
...
import { METHOD_ICONS } from '#/frontend/features/booking/v2/MethodChoice'
import { v2TypesQuery } from '#/frontend/features/booking/v2/queries'
import { useLanguage } from '#/frontend/i18n/language-provider'
import { SplitWords, useReveal, useTilt } from '#/frontend/motion'

/**
 * `/booking` read from Backend2: the same page, with each type naming the
 * ways it can be held. Every V2 type is free (`docs/v2/booking.md`).
 */
export function BookingTypesPageV2() {
    ...
    <section ref={ref} data-reveal-scope="" className="py-section lg:py-section-lg">
      <Container className="flex flex-col gap-10">
        ...
        <p className="text-foreground/55 text-sm">
          ...
          <Link to="/$lang/contact" params={{ lang: language }} className="link-underline-slide text-primary font-semibold">
...
function TypeCard({ type }: { type: PublicBookingType }) {
  const { language } = useLanguage()
  const copy = getBookingCopy(language)
  ...
  return (
    <div data-tilt ref={tilt} className="surface-card surface-card-hover flex flex-col gap-4 rounded-[1.75rem] p-6">
      <div className="flex flex-wrap items-center gap-3">
        <span className="flex items-center gap-1.5">
          {type.methods.map((method) => {
            ...
            return (
              <span key={method} className="brand-mark text-primary" title={words.methods[method]}>
                <Icon aria-hidden="true" className="size-4" />
                <span className="sr-only">{words.methods[method]}</span>
              </span>
            )
          })}
        </span>
      ...
      <div className="flex flex-col gap-2">
        ...
        {type.description ? <p className="text-foreground/58 text-sm leading-7">{type.description}</p> : null}
      </div>
